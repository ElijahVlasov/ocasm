...
let rec multiline_comment st k =
  ...
  else if Char.is_eof ch then error st.lexer_state Unfinished_comment
  else multiline_comment st k
...
let multiline_comment_start st k =
  ...
  else error st.lexer_state @@ Expected_vs_got ('*', next_ch)
...
let consume_number st is_digit builder =
  ...
  else error st.lexer_state @@ Wrong_char_in_number_literal ch
...
let number_start st ch =
  ...
  if ch = '0' then
    ...
    | ch -> error st.lexer_state @@ Wrong_char_in_number_literal ch
  else
...
let read_escaped k st builder =
  ...
  | 'x' ->
      ...
      |> Option.value_or_thunk ~default:(fun () ->
             error ~k st.lexer_state @@ Incorrect_hex_escape_sequence (fst, snd))
  | '"' -> '"'
  | ch -> error ~k st.lexer_state @@ Incorrect_escape_sequence ch
...
let rec read_string_literal st builder =
  ...
  | '\n' ->
      warning ~k st.lexer_state Newline_in_string_literal;
      Token_builder.add_char builder '\n';
      read_string_literal st builder
  | '\x00' -> fail st.lexer_state Unfinished_string_literal
  | ch ->
      Token_builder.add_char builder ch;
      read_string_literal st builder
  ...
  try
    ...
    if has_advanced then return_info_simple White_space
      ...
      | ch ->
          ...
          |> Option.value_or_thunk ~default:(fun () ->
                 error st.lexer_state @@ Junk_symbol ch)
          |> return_info_simple
  with
...
let create isa_m inp_m inp dgn_handler_m dgn_handler =
  {
    isa_m;
    lexer_state = Lexer_state.create inp_m inp dgn_handler_m dgn_handler;
    recovery = None;
  }
