...
module Mk (Isa_token : Isa_token.S) (Input : Input.S) = struct
  ...
  *)
  let return_info_simple st token =
    let module T = Token.MkToken (Isa_token) in
  ...
  let number_start st ch =
    ...
    if ch = '0' then
      ...
      | 'x' ->
          ...
          Token.Hex number
          |> return_info_real st (Token_builder.contents builder)
      | 'b' ->
          Lexer_dsl.skip st.dsl;
  ...
  let read_proper_name st
      (builder : Token_builder.case_sensitive Token_builder.t) =
  ...
  let name_like_started ~isa_specific ?default st ch =
  ...
  let next_aux st =
    ...
    if has_advanced then return_info_simple st White_space
      ...
      | 'A' .. 'Z' | 'a' .. 'z' | '_' ->
          name_like_started ~isa_specific:Isa_token.name st ch
      | '.' ->
          name_like_started ~isa_specific:Isa_token.directive ~default:Dot st ch
      | '%' ->
          name_like_started ~isa_specific:Isa_token.reserved ~default:Percent st
            ch
